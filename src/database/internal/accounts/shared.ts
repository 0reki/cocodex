...
export const OPENAI_ACCOUNT_PLATFORMS = [
  "windows",
  "linux",
  "darwin",
  "all",
] as const satisfies readonly OpenAIAccountPlatform[];
...
export function normalizeOpenAIAccountPlatform(
  platform: string,
): OpenAIAccountPlatform | null {
  const normalized = platform.trim().toLowerCase().replaceAll(" ", "_");
  return OPENAI_ACCOUNT_PLATFORMS.find((item) => item === normalized) ?? null;
}
...
export const OPENAI_ACCOUNT_COLUMNS = `
  id, email, account_id, status, platform, id_token, access_token,
  refresh_token, created_at, updated_at
`;

export function mapOpenAIAccountRow(
  row: OpenAIAccountRow,
): OpenAIAccountRecord {
  const status = normalizeOpenAIAccountStatus(row.status ?? "") ?? "inactive";
  const platform = normalizeOpenAIAccountPlatform(row.platform ?? "") ?? "all";
