...
type SearchRouteDependencies = OpenAIRequestPreparationDependencies &
...
export function registerSearchRoutes(
  ...
  app.post("/v1/alpha/search", async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (!wasAborted && !res.headersSent) {
        const passthrough = deps.buildPassthroughUpstreamError({
          ...
          errorPayload,
          fallbackCode: "search_proxy_failed",
          fallbackMessage: errorMessage ?? "Failed to reach upstream Search API",
        });
      ...
      try {
        ...
        await persistOpenAIResponseLog({
          ...
          fallbackErrorCode: errorMessage ? "search_proxy_failed" : null,
